...
- (instancetype)initWithName:(NSString *) shaderName andCore:(core_crocods_t*)core0
{
    self = [super init];
    if (self) {
        // Program
        ...
        NSString *shaderSource =[[self class] shaderSourceForName:shaderName];
        if (shaderSource==nil) {
            shaderSource =[[self class] shaderSourceForName:@"NearestNeighbor"];
        }
        fragment_shader = [fragment_shader stringByReplacingOccurrencesOfString:@"{filter}"
                                                                     withString:shaderSource];

        fragment_shader = [fragment_shader stringByReplacingOccurrencesOfString:@"{width}"
                                                                     withString:[NSString stringWithFormat:@"%d", core0->screenBufferWidth]];
        fragment_shader = [fragment_shader stringByReplacingOccurrencesOfString:@"{height}"
                                                                     withString:[NSString stringWithFormat:@"%d", core0->screenBufferHeight]];
...
- (void) renderBitmap: (void *)bitmap previous:(void*) previous inSize:(NSSize)size scale: (double) scale
{
    ...
    width = _gb->screenBufferWidth;
    height = _gb->screenBufferHeight;
    ...
    glBindTexture(GL_TEXTURE_2D, texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, bitmap); // GL_UNSIGNED_BYTE
    glUniform1i(texture_uniform, 0);
    glUniform1i(mix_previous_uniform, previous != NULL);
    if (previous) {
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, previous_texture);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, previous); // GL_UNSIGNED_SHORT_5_6_5
