...
apps_menu_publicmenu apps_menu_publicmenus[APPS_MENU_PUBLICMENU_COUNT] = {
    ...
    {ID_SHOW_INFOPANEL_SHORTCUTS, "Shortcuts",                                     NULL,                                            0                               },
    ...
    {ID_KEY_MENU,                 "Keyboard Emulation",                            NULL,                                            0                               },

    {ID_KEY_KEYBOARD,             "No emulation",                                  NULL,                                            0                               },
    {ID_KEY_JOYSTICK,             "Joystick",                                      NULL,                                            0                               },
    {ID_KEY_KEYPAD,               "Keypad",                                        NULL,                                            0                               },
    {ID_SHOW_VIRTUALKEYBOARD,     "Show Virtual Keyboard",                         NULL,                                            0                               },
    {ID_REDEFINE_MENU,            "Custom",                                        NULL,                                            0                               },
    ...
    {ID_EXIT,                     "Quit",                                          NULL,                                            0                               },
    {ID_SAVESCREEN,               "Capture screenshot",                            NULL,                                            0                               },
    {ID_EXPORTDISK,               "Export disk",                                   NULL,                                            0                               }
};
...
void DispMenu(core_crocods_t *core, u16 keys_pressed0)
{
    ...
    for (i = 0; i < folder->nbr; i++) {
        ...
        if (i == apps_menu_mouseSelected) {
// if (core->selectedMenu == firstMenu) {
            keys_pressed = keys_pressed | KEY_A;
// }

            core->selectedMenu = firstMenu;

            apps_menu_mouseSelected = -1;
        }

        firstMenu = firstMenu->nextSibling;
    }

    if (apps_menu_mouseSelected != -1) { // User doesn't select an entry
        keys_pressed = keys_pressed | KEY_B;
        apps_menu_mouseSelected = -1;
    }

    // 1234567890ABCDEFGHIJKJIHGFEDCBA0987654321
    cpcprint16_6w(core, core->overlayBitmap, 320, 4, 156, " Arrows & button A to select ", RGB565(0xFF, 0x00, 0x00), RGB565(0x00, 0x00, 0x00), 1, 1);
    ...
    if (core->ipc.touchDown == 1) {
        ...
        y = core->ipc.touchYpx - core->overlayBitmap_posy - 34;

        ddlog(core, 2, "browse X: %d,%d\n", core->ipc.touchXpx, core->overlayBitmap_posx);
        ddlog(core, 2, "browse Y: %d,%d\n", core->ipc.touchYpx, core->overlayBitmap_posy);

        if ((x >= 0) && (x < 226)) {
            apps_menu_mouseSelected = y / 9;
        } else if ((x >= 226) && (x < 245)) {
        } else {
            keys_pressed = keys_pressed | KEY_B;
        }
...
void apps_menu_init(core_crocods_t *core)
{
    ...
    AddMenu(&root, ID_SHOW_INFOPANEL_SHORTCUTS, 6, 4);
...
// }
    ...
    AddMenu(id, ID_KEY_KEYBOARD, 3, 0);
    ...
    AddMenu(id, ID_SAVESCREEN, 7, 2);
    AddMenu(id, ID_EXPORTDISK, 7, 3);
    ...
    AddMenu(&root, ID_SHOW_VIRTUALKEYBOARD, 3, 0);
