...
void DispKeyboard(core_crocods_t *core, u16 keys_pressed0)
{
    ...
    if (core->ipc.touchDown == 1) {
        ...
        x = core->ipc.touchXpx - core->overlayBitmap_posx;
        y = core->ipc.touchYpx - core->overlayBitmap_posy;

//        printf("after: %d,%d\n", x, y);

        if ((x >= 0) && (x < 320) && (y >= 51) && (y < 222)) {
            int n;
