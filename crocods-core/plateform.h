...
#ifndef CROCOVERSION
#define CROCOVERSION "CROCOVERSION not set"
#endif
...
typedef struct core_crocods_s {
    ...
    char standalone;     // Standalone Apps
    ...
    u8 printer_port;
    u8 printerBuffer[512000];
    u32 printerLength;

    u8 inConsole;
    u16 *consoleBitmap;
    u16 frameBeforeConsole;

    u16 *overlayBitmap;
    u16 overlayBitmap_width, overlayBitmap_height;
    u16 overlayBitmap_posx;     // 0 à 384
    u16 overlayBitmap_posy;     // 0 à 272
    ...
    u8 app_clav[16];     // Matrice du clavier utilisé dans les app

    u8 cliMode;
    u8 cliAutorun;     // autorun first file in directory
    u16 cliStopAfter;
    u8 cliAutoload;     // 1: load file in place of run
    u8 cliVerbose;    // 0: normal, 1: verbose, 2: debug
    u8 cliSaveSnaphsot;         // Save snapshot - to use with cliStopAfter
    u8 cliSaveImage;    // Save image - to use with cliStopAfter
    u8 cliSpeedTest;
    u8 cliGetInfo;
    u8 cliFullScreen;
    u8 cliWarp;
...
/// @brief
/// @param core
/// @param logLevel 0: normal, 1: verbose, 2: debug
/// @param fmt
/// @param
