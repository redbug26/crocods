...
//    ddlog(core, 2, "WritePort: %04X\n", port);
    ...
    if (!(port & 0x1000) ) {            // PRN
        ...
        if (!(core->printer_port & 0x80)) { // only grab data bytes; ignore the strobe signal
            if (core->printerLength < 512000) {
                core->printerBuffer[core->printerLength] = core->printer_port & 0x7f;
                core->printerLength++;
            }
        }
