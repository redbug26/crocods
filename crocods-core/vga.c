...
// #define CK_MAXMEM 0x20000
#define CK_MAXMEM    (2 * 0x10000)
...
//    printf("Call SetMemCPC: bloc: %d, ramselect: %d, numromext: %d\n", core->Bloc, core->RamSelect, core->NumRomExt);
} // SetMemCPC
...
//                printf("Change decodeur: %02X (mode: %d)\n", core->DecodeurAdresse, val & 3);
            }
...
//            printf("Change bloc: %d, ramselect: %d, numromext: %d (port: %04X, val: %02X)\n", core->Bloc, core->RamSelect, core->NumRomExt, port, val);
...
*
********************************************************** !0! ****************/
void WriteROM(core_crocods_t *core, u8 val)
{
    if (val >= 128) {
        core->NumRomExt = val & 31;
    } else {
        core->NumRomExt = val;
    }
    ...
    // printf("Change rom (writeRom): %d,%d\n", core->NumRomExt, val);
}
...
{
    core->MemCPC = malloc(CK_MAXMEM); // 128Ko

    if (/* DISABLES CODE */ (1) == 0) { // use rom
        int n;
        for (n = 0; n < 8; n++) {
            if (n == 0) {
                memcpy(core->MemCPC, core->ROMINF, 16384);
            } else {
                memcpy(core->MemCPC + 16384 * n, core->ROMEXT[n - 1], 16384);
            }
        }
    } else {
        core->hw = CPC_HW_CPC6128;

        memcpy((void *)(cpc6128_bin + 0x5eca), "\xED\xFA\xC9", 3);  // BYPASS tron
