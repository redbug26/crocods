...
void apps_infopanel_init(core_crocods_t *core, int section)
{
    ddlog(core, 2, "apps_infopanel_init: %s\n", core->openFilename);
    ...
    apps_infopanel_section = section;
...
//    cpcprint16(core, core->overlayBitmap, 320, 0, 53, "title (azerty)", RGB565(0xFF, 0x00, 0x00), RGB565(0x00, 0x00, 0x00), 1, 1);
    ...
    if (apps_infopanel_section == 1) {

        for (y = 0; y < 168; y++) {
            memcpy(pdwAddr, core->shortcuts + y * 256, 256 * 2);
            pdwAddr += 320;
        }
        ...
        cpcprint16_6w(core, core->overlayBitmap, 320, 4, 156, "          Button A to leave page         ", RGB565(0xFF, 0x00, 0x00), RGB565(0x00, 0x00, 0x00), 1, 1);
