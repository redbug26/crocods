...
static void UpdatePSG(core_crocods_t *core)
{
    switch ( core->modePSG ) {
        case 2: // MODE_WRITE
            ...
            // core->RegsPSG[core->RegPSGSel] = core->Output[ 0 ]; // Only for the snapshot
...
int ReadPPI(core_crocods_t *core, int adr)
{
    switch ( ( adr >> 8 ) & 3 ) {
        ...
        case 1:  // 0xF5xx - PORT B
        {
            ...
            Data =  (CONSTRUCTEUR << 1);   // BIT 3-2-1
            Data |= (REFRESH_HZ == 50) ? 0x10 : 0x00; // BIT 4

#ifdef USE_TAPE
            Data |= BitTapeIn;    // BIT 7 - CAS.IN
#endif
//                     ); // Port B toujours en lecture

            Data |= ((core->CRTC_Flags & CRTC_VS_FLAG) != 0) ? (1 << 0) : 0;  // BIT 0
